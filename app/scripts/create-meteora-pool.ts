...
async function main() {
  ...
  try {
    ...
    console.error("Error creating pool:", e instanceof Error ? e.message : String(e));
...
async function findExistingPool(connection: Connection, tokenX: PublicKey, tokenY: PublicKey) {
  try {
    const pairs = await DLMM.getLbPairs(connection, "devnet");
    for (const pair of pairs) {
      const account = pair.account;
