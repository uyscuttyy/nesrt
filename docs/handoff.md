...
11. **Pyth On-Chain Validation (code-complete, NOT yet activated)** — `programs/vault` now depends on `pyth-solana-receiver-sdk 0.1`; Deposit/Withdraw take a trailing `price_update` account and enforce TSLAx/USD feed id (`47a15647…`) + 60s staleness + Full verification (`BadPriceFeed` 6014, `StaleOracle` 6015). Two deliberate deviations: (a) `UncheckedAccount` + manual deserialize because the SDK pins the *mainnet* receiver id and its owner check rejects all devnet updates; (b) xStocks TSLAx feed, not Equity TSLA (matches collateral, 24/7). Posting flow is built but inert: `POST /api/pyth/post` fetches the VAA (plain Hermes REST, no bearer header) and builds post_update ixs via request-time SDK load; `app/src/pyth.ts` prepends them + co-signs the ephemeral account in deposit/withdraw, falling back to the legacy path when Hermes 401s. **Do NOT upgrade yet**: Hermes v1+v2+stream update endpoints all 401 without an API key, and `@pythnetwork/pyth-solana-receiver` is unimportable in this repo's npm tree (transitive rpc-websockets@9.3.9 dropped `./dist/lib/client`; 7.x breaks web3@1.99 API) — the route degrades to a clean 502. Activating needs a Hermes key (+ optional dep-tree fix), then upgrade. Frontend builders/tests already pass the trailing account (old deployments ignore extras).

12. **Helius Webhooks (endpoint live, delivery pending)** — `POST /api/webhooks/helius` parses Enhanced payloads, filters vault program touches, decodes deposit/withdraw from log lines (RPC-direction fallback), stores a 200-event ring; `GET` serves same-origin feed. `history.ts` merges webhook-first + RPC fallback, deduped. Optional `HELIUS_WEBHOOK_SECRET` (`authorization: Bearer` or `?auth=`). Live delivery needs a public URL + Helius dashboard registration (localhost unreachable); until then the RPC path carries history. In-memory store = dev only (use KV/DB in production).
