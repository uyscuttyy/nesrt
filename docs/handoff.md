...
11. **Pyth On-Chain Validation (ACTIVE via devnet stub)** — `programs/vault` depends on `pyth-solana-receiver-sdk 0.1`; Deposit/Withdraw take a trailing `price_update` and enforce TSLAx/USD feed id (`47a15647…`) + staleness + Full verification (`BadPriceFeed` 6014, `StaleOracle` 6015). Deviations: (a) `UncheckedAccount` + manual deserialize (SDK pins *mainnet* receiver id); (b) xStocks TSLAx feed, not Equity TSLA. **Bypass (Option 2, live):** dual-path validation accepts a fresh Hermes update (60s) OR the admin-posted stub PDA `2cXuUBQRjeDTJMCXVrq17VrS36spJcyXhxP6AE3HfKoZ` (feed id + 24h window, `update_stub_price` admin-gated). Upgraded + E2E-verified: 8/8 tests (stub post, gated deposit, drip, gated withdraw + skim). Hermes pull flow (`POST /api/pyth/post`, prepend + co-sign) is built but inert until a key exists (all update endpoints 401).
