...
11. **Pyth On-Chain Validation (code-complete, NOT yet activated)** — `programs/vault` now depends on `pyth-solana-receiver-sdk 0.1`; Deposit/Withdraw take a trailing `price_update` account and enforce TSLAx/USD feed id (`47a15647…`) + 60s staleness + Full verification (`BadPriceFeed` 6014, `StaleOracle` 6015). Two deliberate deviations: (a) `UncheckedAccount` + manual deserialize because the SDK pins the *mainnet* receiver id and its owner check rejects all devnet updates; (b) xStocks TSLAx feed, not Equity TSLA (matches collateral, 24/7). **Do NOT upgrade yet**: no Hermes key exists to post pull updates and the stored feed address was never created — activating now would brick deposits AND withdrawals (user funds!). Activation needs: Hermes API key → frontend posts `post_update` + passes the update account (`NEXT_PUBLIC_PYTH_PRICE_UPDATE`) → then upgrade. Frontend builders/tests already pass the trailing account (old deployments ignore extras).
